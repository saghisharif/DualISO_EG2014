...
public:
  ...
  std::vector<TiffIFD*> mSubIFD;
  std::map<TiffTag, TiffEntry*> mEntry;
  ...
  TiffEntry* getEntryRecursiveWhere(TiffTag tag, std::string isValue);
  std::vector<TiffIFD*> getIFDsWithTagWhere(TiffTag tag, std::string isValue);
  std::vector<TiffIFD*> getIFDsWithTagWhere(TiffTag tag, uint32 isValue);
  Endianness endian;
